...
public class DrawerFragment extends Fragment implements OnNavigationItemSelectedListener {
    ...
    private static final String TAG_WEB = WebFragment.class.getSimpleName();
    ...
    public void onGetSubjectIdEvent(GetSubjectEvent event) {
        ...
        ActivityOptionsCompat options = ActivityOptionsCompat.
                makeSceneTransitionAnimation(getActivity(), getView().findViewWithTag(event.getSubject().getName()), "subject_cover");
    ...
    private void setupUser() {
        ...
        View headerView = mNavigationView.getHeaderView(0); // new API for 23.1.1

        RoundedImageView mUserAvatar = (RoundedImageView) headerView.findViewById(R.id.user_avatar);
        TextView mUserNickname = (TextView) headerView.findViewById(R.id.user_nickname);
        TextView mUserSign = (TextView) headerView.findViewById(R.id.user_sign);

        if (user == null) {
            mUserAvatar.setVisibility(View.INVISIBLE);
            mUserNickname.setVisibility(View.INVISIBLE);
            mUserSign.setVisibility(View.INVISIBLE);
        } else {
            ...
            mUserSign.setText(user.getSign());
            Picasso
                    .with(getActivity())
    ...
    private void goToDst(String tag) {
        ...
        if (targetFragment == null) {
            ...
            if (tag.equals(TAG_LOGIN)) {
                ...
                targetFragment = new WebFragment();
            }

            if(targetFragment != null){
                Fade fadeTransition = new Fade();
                fadeTransition.setDuration(400);
                targetFragment.setEnterTransition(fadeTransition);
            }
